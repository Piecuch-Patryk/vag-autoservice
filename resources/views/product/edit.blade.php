	...
	<div class="row">
		...
		<form method="POST" action="{{ route('product.update', $product->id) }}" class="pt-5">
			...
			<div class="input-group input-group-sm mb-3">
				...
				<input value="{{ old('proName') ? old('proName') : $product->proName }}" name="proName"
					type="text" class="form-control form-control-sm" id="InputName"
					aria-describedby="nameHelp">

				<input
					value="{{ old('price') ? number_format(old('price') / 100, 2) : number_format($product->price / 100, 2) }}"
